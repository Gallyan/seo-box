...
function get_crawl_unique_quotidien($div_container){
    ...
    $res = $dbh->query($qry);
    while($row = $res->fetch(PDO::FETCH_OBJ)){
        $serie_tab[] = intval($row->nb)-2;
    }
    $series['Autres'] = $serie_tab;

	return(get_stacked_areas($div_container, $x_tab, $series, 'Crawl unique quotidien', 'Googlebot'));
