...
function add_log($adresse_ip, $referer, $user_agent, $url, $res_code=200, $response_time=0){
	global $dbh;
			...
			if(strstr($user_agent,'Googlebot/2.1') && strstr($adresse_ip, '66.249')){
				$type_id = 0;
			}elseif(strstr($user_agent,'Googlebot-Image/1.0') && strstr($adresse_ip, '66.249')){
                $type_id = 2;
            }elseif(strstr($user_agent,'bingbot/2.0')){
                $type_id = 3;
            }elseif(strstr($user_agent,'MJ12bot/v1.4.7')){
                $type_id = 4;
            }elseif(strstr($user_agent,'SemrushBot')){
                $type_id = 5;
            }elseif(strstr($user_agent,'Qwantify/2.4w') && strstr($adresse_ip, '194.187')){
                $type_id = 6;
            }elseif(strstr($user_agent,'archive.org_bot') && strstr($adresse_ip, '207.241')){
                $type_id = 7;
            }elseif(strstr($user_agent,'Baiduspider/2.0')){
                $type_id = 8;
            }
